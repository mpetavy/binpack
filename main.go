...
func init() {
	common.Init(false, "1.0.0", "", "", "2018", "Tool to embedd resource files into go source files", "mpetavy", fmt.Sprintf("https://github.com/mpetavy/%s", common.Title()), common.APACHE, nil, nil, nil, run, 0)
...
func run() error {
	...
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("			common.Error(fmt.Errorf(\"multiple resources with name %s found: %s\", name, l))\n", "%%s", "%%v")))
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("		}\n")))
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("\n")))
	...
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("\n")))
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("    b := common.FileExists(filepath.Base(this.File))\n")))
	...
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("      b = common.FileExists(this.File)\n")))
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("      if b {\n")))
	...
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("				common.Error(i.Close())\n")))
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("			}()\n")))
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("\n")))
	...
	common.Ignore(fmt.Fprintf(goFile, fmt.Sprintf("			if !common.FileExists(path) {\n")))
